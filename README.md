...
- [The official MCP registry, scanned statically (September 2026)](./docs/research/registry-2026-09/README.md) — every server in `registry.modelcontextprotocol.io`, rendered to a client config and run through guardmcp's static rules. Generated by [`scripts/registry-scan.mjs`](./scripts/registry-scan.mjs), with the raw snapshot and findings alongside so the numbers can be re-derived.
