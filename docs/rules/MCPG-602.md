...
- For `docker`/`podman` entries, a `--host 0.0.0.0`-style flag or variable is not flagged unless the container runs with `--network host`: on its own network a container must bind all interfaces for a published port to work, and what reaches the network is the host side of `-p`, which is checked. The entry's `env` block belongs to the runner, not the container, so it is not read either; a value passed through with `-e NAME` is not followed.
